...
const hamburgerimenu = document.querySelector('.nav-list2');
...
function openHam() {
  ...
  hamburgerimenu.setAttribute('style', 'display:block');
 ...
 */
function closeham() {
  hamburgerimenu.setAttribute('style', 'display:none');
...
function closelist() {
  hamburgerlist.forEach((item) => {
    item.addEventListener('click', closeham);
  });
}
closelist();
...
function cheffun() {
  featuredChefs.forEach((chefy) => {
    bolechefs.innerHTML += `<div class="${chefy.type}">
